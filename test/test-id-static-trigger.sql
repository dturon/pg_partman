...
CREATE TABLE partman_test.z_test1_serial(a serial primary key,data int);
...
BEGIN
    ...
    IF TG_TABLE_NAME!='z_test1_serial' THEN
...
BEFORE INSERT
ON partman_test.z_test1_serial 
FOR EACH ROW 
...
-- indexes
CREATE INDEX ON partman_test.z_test1_serial(data);



--partitions
SELECT * FROM create_parent(p_parent_table:='partman_test.z_test1_serial', p_control:='a', p_type:='id-static', p_interval:='100000', p_premake:=4, p_use_run_maintenance:=NULL::boolean, p_inherit_fk:=true, p_jobmon:=false, p_debug:=false);

--trigger tests
INSERT INTO partman_test.z_test1_serial SELECT 1, random()*1000;
SELECT results_eq(
...
SELECT results_eq(
    ...
    ARRAY['z_test1_serial_p0'], 'Check where fired trigger on BEFORE INSERT events'
);
...
SELECT results_eq(
    ...
    ARRAY['z_test1_serial_p0'], 'Check where fired trigger on AFTER INSERT events'
);


UPDATE partman_test.z_test1_serial SET data=5 WHERE a=1;
SELECT results_eq(
...
SELECT results_eq(
    ...
    ARRAY['z_test1_serial_p0'], 'Check where fired trigger on BEFORE UPDATE events'
);
...
SELECT results_eq(
    ...
    ARRAY['z_test1_serial_p0'], 'Check where fired trigger on AFTER UPDATE events'
);

DELETE FROM partman_test.z_test1_serial WHERE a=1;
SELECT results_eq(
...
SELECT results_eq(
    ...
    ARRAY['z_test1_serial_p0'], 'Check where fired trigger on BEFORE DELETE events'
);
...
SELECT results_eq(
    ...
    ARRAY['z_test1_serial_p0'], 'Check where fired trigger on AFTER DELETE events'
);

--test move data
TRUNCATE partman_test.events;
INSERT INTO partman_test.z_test1_serial SELECT 10000000, random()*1000;

SELECT run_maintenance();
SELECT results_eq(
    'SELECT count(*)::int FROM ONLY partman_test.z_test1_serial;',
    ARRAY[1], 'Check count rows in parent'
);

--move data
SELECT partition_data_id('partman_test.z_test1_serial');
...
SELECT results_eq(
    'SELECT count(*)::int FROM partman_test.z_test1_serial_p10000000;',
...
TRUNCATE partman_test.events;
TRUNCATE partman_test.z_test1_serial;

--drop other triggers on parrent 
DROP TRIGGER trg_before_insert ON partman_test.z_test1_serial;
DROP TRIGGER trg_after_insert ON partman_test.z_test1_serial;
DROP TRIGGER trg_before_update ON partman_test.z_test1_serial;
DROP TRIGGER trg_after_update ON partman_test.z_test1_serial;
DROP TRIGGER trg_before_delete ON partman_test.z_test1_serial;
DROP TRIGGER trg_after_delete ON partman_test.z_test1_serial;
...
SELECT reapply_triggers('partman_test.z_test1_serial', p_delete_not_exists:=True);
...
SELECT results_eq(
    E'SELECT ev_table FROM partman_test.events WHERE ev_op=\'INSERT\' AND ev_when=\'AFTER\';',
    ARRAY['z_test1_serial'], 'Check where fired trigger on AFTER INSERT events'
);
...
SELECT undo_partition_id('partman_test.z_test1_serial', p_keep_table := false);


SELECT hasnt_table('partman_test', 'z_test1_serial_p0', 'Check z_test1_serial_p0 doesn''t exists anymore');
