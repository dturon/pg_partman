...
LOOP
    ...
    IF v_row.type = 'time-static' OR v_row.type = 'time-dynamic' OR v_row.type = 'time-custom' THEN
        ...
        v_premade_count = COALESCE(round(EXTRACT('epoch' FROM age(v_last_partition_timestamp, v_current_partition_timestamp)) / EXTRACT('epoch' FROM v_row.part_interval::interval)),0);
        IF v_last_partition_timestamp IS NULL THEN
            v_last_partition_timestamp = current_date-v_row.part_interval::interval;
            IF v_row.part_interval LIKE '%month%' THEN
                v_last_partition_timestamp = date_trunc('month',v_last_partition_timestamp);
            ELSIF v_row.part_interval LIKE '%year%' THEN
                v_last_partition_timestamp = date_trunc('year',v_last_partition_timestamp);
            END IF;
        END IF;

        v_next_partition_timestamp := v_last_partition_timestamp;
        RAISE DEBUG 'v_premade_count: %, v_next_partition_timestamp: %', v_premade_count, v_next_partition_timestamp;
